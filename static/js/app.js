...
async function loadPortfolio() {
  ...
  document.getElementById("portfolio-summary").innerHTML = `
    <table class="table table-sm mb-0">
      ...
      <tbody>
        <tr><td><strong>Wrestling</strong></td><td>${w.count}</td><td>${fmt(w.cost)}</td><td>${fmt(w.value)}</td>
          <td class="${w.value-w.cost>=0?'gain':'loss'}">${(w.value-w.cost>=0?"+":"")+fmt(w.value-w.cost)}</td></tr>
        <tr><td><strong>Soccer</strong></td><td>${s.count}</td><td>${fmt(s.cost)}</td><td>${fmt(s.value)}</td>
          <td class="${s.value-s.cost>=0?'gain':'loss'}">${(s.value-s.cost>=0?"+":"")+fmt(s.value-s.cost)}</td></tr>
        <tr class="table-dark"><td><strong>Total</strong></td><td>${t.count}</td><td><strong>${fmt(t.cost)}</strong></td>
          <td><strong>${fmt(t.value)}</strong></td>
          <td class="${t.value-t.cost>=0?'gain':'loss'}"><strong>${(t.value-t.cost>=0?"+":"")+fmt(t.value-t.cost)}</strong></td></tr>
      </tbody>
    </table>`;
  loadPortfolioChart();
}
...
async function loadPortfolioChart() {
  ...
  if (filter === "all" || filter === "wrestling") {
    datasets.push({ label: "Wrestling", data: history.map(h => h.wrestling), borderColor: "#dc3545", backgroundColor: "rgba(220,53,69,0.1)", tension: 0.3, fill: false });
  }
  if (filter === "all" || filter === "soccer") {
    datasets.push({ label: "Soccer", data: history.map(h => h.soccer), borderColor: "#0d6efd", backgroundColor: "rgba(13,110,253,0.1)", tension: 0.3, fill: false });
  }
  if (filter === "all") {
    datasets.push({ label: "Total", data: history.map(h => h.total), borderColor: "#198754", backgroundColor: "rgba(25,135,84,0.1)", tension: 0.3, fill: true, borderWidth: 2 });
  }
  ...
  setTimeout(() => {
    ...
    state.historyChart = new Chart(ctx, {
      ...
      data: {
        ...
        datasets: [{ label: "Value", data: history.map(h => h.value), borderColor: "#0d6efd", backgroundColor: "rgba(13,110,253,0.1)", tension: 0.3, fill: true }]
      },
      options: {
        responsive: true,
...
async function openEditModal(type, id) {
  state.editId = id;
  state.editType = type;
  const card = await fetch(`/api/${type}?q=`).then(() => null).catch(() => null);
  // Fetch via list with no filter but we need a single card — use a direct approach
...
function wrestlingForm(c) {
  return `
    <div class="row g-2">
      <div class="col-md-6"><label class="form-label">Wrestler Name *</label>
        <input class="form-control" id="f-wrestler_name" value="${esc(c.wrestler_name||"")}"></div>
      <div class="col-md-6"><label class="form-label">Brand</label>
        ...
        <datalist id="brand-list"><option value="Raw"><option value="SmackDown"><option value="NXT"><option value="AEW"><option value="WCW"><option value="ECW"><option value="WWF"></datalist></div>
      <div class="col-md-6"><label class="form-label">Card Type</label>
        <input class="form-control" id="f-card_type" list="type-list" value="${esc(c.card_type||"")}">
        <datalist id="type-list"><option value="Base"><option value="Refractor"><option value="Auto"><option value="Prizm"><option value="Patch"><option value="Rookie"><option value="Parallel"><option value="Gold"><option value="Silver"><option value="Bronze"></datalist></div>
      <div class="col-md-6"><label class="form-label">Card Number</label>
        <input class="form-control" id="f-card_number" value="${esc(c.card_number||"")}"></div>
      <div class="col-md-6"><label class="form-label">Cost ($)</label>
        <input class="form-control" type="number" step="0.01" id="f-cost" value="${c.cost||0}"></div>
      <div class="col-md-6"><label class="form-label">Current Value ($)</label>
        <input class="form-control" type="number" step="0.01" id="f-current_value" value="${c.current_value||0}"></div>
      <div class="col-12"><label class="form-label">Notes</label>
        <textarea class="form-control" id="f-notes" rows="2">${esc(c.notes||"")}</textarea></div>
    </div>`;
}
...
function soccerForm(c) {
  return `
    <div class="row g-2">
      <div class="col-md-6"><label class="form-label">Player Name *</label>
        <input class="form-control" id="f-player_name" value="${esc(c.player_name||"")}"></div>
      <div class="col-md-6"><label class="form-label">Team</label>
        <input class="form-control" id="f-team" value="${esc(c.team||"")}"></div>
      <div class="col-md-6"><label class="form-label">League</label>
        <input class="form-control" id="f-league" list="league-list" value="${esc(c.league||"")}">
        <datalist id="league-list"><option value="Premier League"><option value="La Liga"><option value="Bundesliga"><option value="Serie A"><option value="Ligue 1"><option value="MLS"><option value="Champions League"></datalist></div>
      <div class="col-md-6"><label class="form-label">Card Type</label>
        <input class="form-control" id="f-card_type" list="type-list2" value="${esc(c.card_type||"")}">
        <datalist id="type-list2"><option value="Base"><option value="Refractor"><option value="Auto"><option value="Prizm"><option value="Patch"><option value="Rookie"><option value="Parallel"></datalist></div>
      <div class="col-md-6"><label class="form-label">Card Number</label>
        <input class="form-control" id="f-card_number" value="${esc(c.card_number||"")}"></div>
      <div class="col-md-6"><label class="form-label">Year</label>
        <input class="form-control" type="number" id="f-year" value="${c.year||""}"></div>
      <div class="col-md-6"><label class="form-label">Cost ($)</label>
        <input class="form-control" type="number" step="0.01" id="f-cost" value="${c.cost||0}"></div>
      <div class="col-md-6"><label class="form-label">Current Value ($)</label>
        <input class="form-control" type="number" step="0.01" id="f-current_value" value="${c.current_value||0}"></div>
    ...
    <nav>
      <ul class="pagination pagination-sm mb-0">
        <li class="page-item ${data.page===1?"disabled":""}">
          <a class="page-link" href="#" onclick="goPage('${tab}',${data.page-1})">‹</a></li>
        ${pageNums(data.page, data.pages).map(p =>
          p === "..." ? `<li class="page-item disabled"><span class="page-link">…</span></li>`
